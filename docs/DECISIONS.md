...
- **Still open.** The other two URLs the user flagged in the same round (`mahe-ba-courses`, `mahe-data-science-and-businesss-analytics-courses`) were confirmed via live re-investigation to be the SAME hub/listing-page pattern as ADR-030's `mahe-ds-courses` (both mention MSc AND PGCP substantially — 16/14 and 43/38 mentions respectively), now three confirmed instances of a `mahe-{subject}-courses` page family. This needs a product/design decision before any code change — three options were presented to the user (report multiple matching candidates instead of one; match against a similar hub page on the Master if one exists; give hub pages their own distinct outcome instead of reusing `ambiguous`) — not yet chosen. `msc-ds-popup` (also re-flagged) was traced to a specific cause — its only surviving differentiator word ("data") is shared with a competing candidate's own genuine, accurate content (a Biostatistics page's real curriculum bullet "Explore Data Science in Healthcare") — and has no safe fix identified without either fabricating evidence or stripping real content from another page; left open rather than forcing something.
