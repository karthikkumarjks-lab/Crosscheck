...
- **Root causes found (six, all in the same subsystem, diagnosed via live testing against the real site).**
...
- **Decision — six additive, registry-aware fixes, all optional/backward-compatible (absent `registry`/new param = zero behavior change for every pre-fix caller):**
...
- **Verification.** Iteratively live-tested against the real site throughout (`online-bba-mahe`, `online-mba-mahe`, `online-bba`, `online-bcom`, `online-mcom`, `mahe.onlinemanipal.com/programs/mba-manipal-academy-of-higher-education`, `mahe.onlinemanipal.com/programs/mba-healthcare-manipal-academy-of-higher-education`, `manipaluniversity.co.in/online-mba-degrees`, `manipaluniversity.co.in/online-mca-degrees`, `manipaluniversity.co.in/online-bba-degrees`) after each fix, re-running the full suite after every change, then the complete real 89-target batch the user supplied:
  - Before any of today's fixes: 43 success / 23 ambiguous / 19 not-found / 4 unreachable (flaky). MUJ's MBA page (score 90) beat MAHE's own genuine MBA page for a MAHE target; `online-mba-mahe`-style targets resolved to the wrong program (a BCom page) or failed outright; every `manipaluniversity.co.in`/`mahe.onlinemanipal.com` target either false-failed or matched the wrong institution; `online-bba`/`online-bcom`/`online-mcom` (which explicitly name "Manipal University Jaipur" in their own program text) tied 3-way ambiguous despite being unambiguous to a human reader.
  - **Final: 61 success / 8 ambiguous / 20 not-found / 0 unreachable.** All of the above now resolve to their correct MAHE/MUJ candidate with a decisive score margin (e.g. 100 vs 40, 85 vs 25, 100 vs 40). The 20 remaining `not-found` targets were individually spot-checked via curl — every one redirects to the Master homepage (dead/retired links from the user's own list, not a matching bug: `program: null` because there is no real page to extract from). The 8 remaining `ambiguous` targets were individually inspected — each is genuine ambiguity where the page's own content truly never specifies which of MUJ/SMU/MAHE it means (a bare BBA-specialization landing page with no institution wording, tying 3-way at the same score) — the correct, honest outcome per this project's "never guess" design principle, not a bug.
  - packages/core: 347/347 tests passing (12 new regression tests added across this session's fixes, 2 stale test fixtures updated to reflect the more precise institution-conflict logic). modules/website-quality: 219/219 tests passing (2 new regression tests added). Zero net regressions across both suites at every step.
- **Not addressed / still open.** Institution matching now depends on `matchSpecificInstitution`'s exact whole-string equality; a target institution mention that's specific but phrased in a form not registered as an alias (e.g. a typo, or an unlisted abbreviation) would still resolve to "unresolved"/ambiguous rather than matched — by design (never guess), but worth revisiting if it recurs. The 20 dead/redirecting links in the user's own 89-URL list are a data-freshness issue on their end, not something CrossCheck can resolve. The user's second, separate ask (an easier way to review ~90 target reports without opening each one individually) is addressed by the existing "All Reports" combined page (`/runs/:runId/report`); not yet re-confirmed with the user whether that fully satisfies the ask.
