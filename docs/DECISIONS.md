...
## ADR-045: Per-page spell check — independent of the Master-vs-Target comparison (2026-09-02)

- **Context.** User's explicit request: "we need to add spell check for all the pages. Dont compare — you say there is spell miss in target and master. If none say 0. Need the count, if I open then it shows the locations." A genuinely new kind of check for this tool — every other field asks "does Target match Master?"; this one asks "does THIS page's own text have spelling mistakes?", computed separately for Master and for Target, with no comparison between the two at all.
- **Decision.** Deterministic, dictionary-based, no LLM (matching the project's existing no-LLM discipline) — Hunspell's `en_US` word list via the `nspell` + `dictionary-en` npm packages. New `checkSpelling()`/`properNounWordsFrom()` in `modules/website-quality/src/understanding/spellCheck.ts`, checking the tool's own already-extracted content (`{fieldKey, text}` pairs built from `claims` + `semanticFacts`) rather than re-scanning raw page HTML — this reuses the exact prose every other field already surfaces (fee text, eligibility, curriculum, accreditation...) and skips nav/footer/cookie-banner chrome nobody asked to have checked. A per-page known-proper-nouns set is derived generically from that same page's own resolved identification (institution/program/degree — see below on why not brand) so a real institution/program name is never flagged just because a generic English dictionary doesn't recognize it. All-uppercase tokens (MAHE, UGC, EMI, NAAC, AICTE...) are auto-exempted as acronyms, institution-agnostic. Result shape: `{count, items: [{word, locations: [{fieldKey, excerpt}]}]}` — `count` is 0 when clean, and each misspelled word carries up to 5 location excerpts so a non-zero count can be drilled into, per the user's explicit spec. Wired into `TargetRunResult.spellCheck: {master, target} | null`, computed once per successful target run alongside `priorityComparison` (no new fetch).
- **Known-proper-nouns scope note.** The known-words set is built from `resolution.identification` (institution/program/degree), not from `understanding.brand` — `understanding` itself is local to `resolveOneTarget` and isn't returned to the caller where spell-check runs; threading a 4th field through just for `brand` wasn't worth it for a set already covering the institution's own name.
- **Verification.** `packages/core`, `modules/website-quality`, `apps/api` all rebuild cleanly. 6 new unit tests (`spellCheck.test.ts`): 0-count clean text, a real misspelling flagged with correct field/excerpt, known-proper-noun exemption, all-caps acronym exemption, repeated-word location grouping. Full suite: 396 (core) + 231 (website-quality) + 94 (dashboard) all passing.
- **Not yet done.** No dashboard UI surfaces `spellCheck` yet (count display, drill-into-locations) — backend/data-model only so far.

---

## ADR-046: Overview table — sticky/horizontally-synced header, and Match/Partial/Unmatch filters per field column (2026-09-02)

- **Context.** Two follow-up UX requests against the run overview table (`TargetTable.tsx`): (1) the header row should stay visible while scrolling down a long target list instead of scrolling out of view, and, once that's a self-scrolling pane, should also track the body horizontally when scrolled right — the table is often wider than the viewport (6 per-field status columns, see ADR-036's addition); (2) each per-field status column (Fee Structure, Eligibility, Specializations, Course Duration, Course Curriculum, Accreditations) needs its own Match/Partial/Needs-review/Unmatch filter dropdown, not just a sort.
- **Decision (sticky header).** Plain `position: sticky; top: 0` on `.target-table thead th`, pinned to the page viewport, does not work here: any ancestor with `overflow-x` set becomes the sticky-containment box for BOTH axes regardless of that ancestor's own `overflow-y` value (a real CSS overflow-box rule, confirmed by direct testing — `overflow-y: hidden` on `.target-table-wrap` didn't fix it either, for the same reason). Rather than fight that, `.target-table-wrap` now has `overflow: auto; max-height: 70vh` — one bounded scroll pane for both axes. This gives exactly what was asked: the header stays pinned as the body scrolls vertically underneath it, AND moves in lockstep with the body on horizontal scroll, because both live in the same scrollport by construction.
- **Decision (per-column filters).** A `<select>` (options: All / Match / Partial / Needs review / Unmatch) added to each priority-field column header, next to its existing sort button (`PriorityColumnHeader` in `TargetTable.tsx`). Filtering is a plain `Array.filter` over `run.perTarget` using the same `priorityComparison.fields`/`secondaryFields` status lookup the column's own badge and sort already use (`priorityFieldStatus`, refactored out of the pre-existing `priorityFieldRank`) — no new data path. Multiple column filters combine as AND. A "Showing N of M targets (filtered)" line appears whenever any filter is active, so a filtered view is never silently mistaken for the full list.
- **Verification.** Live-tested against a real 10-target Online Manipal run: confirmed the header pins on vertical scroll and tracks the body on horizontal scroll (screenshots + computed-style checks); confirmed the Specializations filter set to UNMATCH correctly narrowed 10 rows to the 4 real UNMATCH rows (`ln-mca-mahe`, `ln-pgcp-ds-mahe`, `ln-pgcp-ba-mahe`, `ln-pgcp-lscm-mahe`) and a filter with zero matches correctly shows "Showing 0 of 10". Full dashboard suite (94 tests, including the pre-existing `TargetTable.test.tsx`) passing unchanged.

---

## ADR-047: Overview table — per-fee-identifier columns replace "Fee Structure"; Spell Check column added; spell-check accuracy fixes (2026-09-03)

- **Context.** User's explicit follow-up: "instead of fee structure we need to add more columns like Full fee payment, Annual fee payment, Semester fee payment, No-cost EMI starting or Monthly payment and discount price for full payment... also i asked for spellcheck column is missing." Two asks: (1) split the overview's single "Fee Structure" column into one column per fee identifier; (2) surface ADR-045's spell-check data (computed since that session but never actually shown anywhere in the dashboard) as its own overview column.
- **Decision (fee columns).** `PriorityComparison` gained a new `feeComponents: FeeComponentRow[]` array — the exact same per-identifier sub-facts `buildFeeStructureField`'s aggregate "Fee Structure" row already computed internally (`resolveFeeComponentSubFacts`), now also exposed individually (same status/masterValue/targetValue/notes/evidence shape as `PriorityFactRow`, so the frontend renders it with the same components, not new markup). Extracted the shared resolution logic into `computeFeeStructureSubFacts` so `buildFeeStructureField` (unchanged signature/return, zero existing-test impact) and `buildPriorityComparison`'s new `feeComponents` field can never disagree about the same underlying fact — computed twice (once per call site) since it's pure, cheap array/regex work, not I/O; not threaded through as an extra parameter, to avoid touching `buildFeeStructureField`'s widely-tested public shape. The overview's single "Fee Structure" column is replaced by 5: Full Fee Payment, Annual Fee Payment, Semester Fee Payment, No-cost EMI / Monthly Payment, Discount (Full Fee) — mapping onto `FEE_COMPONENTS`' "Full Fee"/"Annual/Yearly Fee"/"Semester Fee"/"Monthly EMI"/"Full Fee (After Discount)" respectively; a component absent for a given program (e.g. no discount) renders "—", never a fabricated status. Each new column gets the same sort + Match/Partial/Needs-review/Unmatch filter dropdown every other priority column already has (ADR-046), generalized via a shared `columnStatus`/`columnRank` dispatch keyed by a `fee:`-prefixed string so fee-component and priority-field columns can share one filter/sort mechanism without colliding. The Target Detail page gained its own "Fee Structure — by identifier" table (reusing `PriorityComparisonTable` unchanged, since `FeeComponentRow` already matches its row shape) directly under the aggregate row, so a user can see exactly which identifier the aggregate PARTIAL/UNMATCH is about.
- **Decision (Spell Check column).** Overview gets a "Spell Check" column showing `M:<count> · T:<count>` (green when both 0, red otherwise), with the actual misspelled words as a hover tooltip. The Target Detail page gained a full `SpellCheckPanel` section (Master/Target side by side, each word with every location's field + excerpt) — "opening" a target, exactly how every other field's detail is already reached, is how the overview's count reaches its locations, per ADR-045's original spec.
- **Decision (spell-check accuracy — found via this session's own live testing, not user-reported).** A real run against `onlinemanipal.com` surfaced 83/33 "misspellings" on one page — live inspection showed nearly all were false positives, not real typos, from 3 distinct causes, each fixed:
  1. **Raw HTML markup in checked text** (`<img src="…">` fragments that leaked into a FEES semantic fact's value) flagged "img"/"src"/"svg" as words. Fixed defensively at the spell-check boundary — `stripHtmlTags()` before tokenizing — rather than chasing why the upstream FEES extraction let raw markup through (a separate, deeper issue, left open).
  2. **Acronym plurals** ("EMIs") weren't covered by the existing all-uppercase acronym exemption (`"EMIs" !== "EMIS"`). Fixed: `isAcronymOrCode` now also strips one trailing lowercase "s" and re-checks.
  3. **Standard British/Indian-English spellings** ("amongst", "programme"...) aren't in `dictionary-en` (US-only) — Indian higher-ed sites use British spelling throughout. **Tried and rejected**: merging in `dictionary-en-gb`'s full word list — nspell needs one dictionary's own affix rules, and reusing en_US's affix rules against en_GB's differently-flag-coded `.dic` entries produced cross-contaminated stemming that started marking a genuine typo ("recieve") as *correct* too, live-confirmed via a direct comparison test — an unacceptable regression to the one thing this feature must never get wrong. Fixed instead with a small, explicit, hand-checked `BRITISH_SPELLING_ALLOWLIST` (~35 words) — no dictionary-merge risk. (A sentence-initial-capitalization theory was tested and disproven along the way — nspell's own `correct()` already normalizes simple capitalization on every real word tried; no fix was needed or added for that.)
- **Verification.** Live re-run of the same real page after all three fixes: Master 83→4, Target 33→4 misspellings — the remaining 4 on each side are genuine dictionary-coverage gaps (informal marketing language "flexi-payment", the brand-ish word "Pharma", the site's own domain name "onlinemanipal" appearing in body text), not bugs; a known, accepted limitation of any generic-dictionary approach, not chased further. 5 new `priorityComparison.test.ts` tests (feeComponents independence/aggregation-agreement) + 3 new `spellCheck.test.ts` tests (British allowlist, acronym-plural, HTML-stripping) + updated `TargetTable.test.tsx`/`priorityComparisonIntegration.test.ts` (old "Fee Structure" column references moved to still-present columns) + defensive `?? []` guards for `feeComponents` on real captured pre-ADR-047 test fixtures. Full suite: 401 (core) + 234 (website-quality) + 96 (dashboard) = 731 passing.
...
## ADR-052: Spell check switches to British English as the primary dictionary; acronym detection generalized past all-caps-only (2026-09-03)

- **Context.** User's explicit standing instruction: "We are using british english so you can check accordingly" — plus three more specific words to ignore ("MSC, DataCamp and flexi") and a question about why "Upskilling" was flagged. This changes the earlier approach (ADR-047/051's `BRITISH_SPELLING_ALLOWLIST`, a hand-curated list of individual British words) into a real, durable dictionary-level decision rather than continuing to patch one word at a time.
- **Decision (dictionary).** Switched the checked dictionary from `dictionary-en` (US) to `dictionary-en-gb` (British) as the ONLY loaded dictionary — not merged with the US one. Merging both was tried again here (to get "either dialect's spelling is accepted") and rejected again, for the exact reason ADR-051 already found: nspell needs one dictionary's own affix rules, and reusing one dialect's affix rules against the other's differently-flag-coded `.dic` entries cross-contaminates stemming and starts accepting real typos. A *single* `dictionary-en-gb` carries none of that risk (one dictionary, one affix file) — verified directly before switching: every common British spelling (amongst, whilst, programme, colour, organise, recognised, centre...) correct, "recieve"/"teh" still correctly wrong, and — the actual point of the switch — American spellings (organize, recognized, specialization, behavior...) now correctly flagged, since British English is genuinely what should be checked. `BRITISH_SPELLING_ALLOWLIST` (35+ hand-picked words, added across ADR-047/051) is deleted entirely — the dictionary itself now carries all of it natively, so the allowlist was pure redundancy once the dialect switched.
- **Decision (generalized acronym detection).** The user's "MSC" report turned out to be the same bug class as ADR-051's "IoA": `isAcronymOrCode` only recognized an ALL-UPPERCASE token, so a conventionally-styled mixed-case degree abbreviation ("MSc", "PhD" — capital-capital-lowercase or capital-lowercase-capital) fell through to the dictionary and got flagged. Rather than allowlist "msc" as one more individual string (this is a higher-ed platform; there are dozens of these — MSc, PhD, BSc, BCom, MCom, LLB, LLM...), generalized the rule: a short (<=6 char) token with 2+ uppercase letters is treated as an acronym/abbreviation, full stop — a real misspelling never spontaneously introduces a second capital letter mid-word, so this never risks hiding a genuine typo. This also makes the earlier one-off "ioa" allowlist entry redundant (removed).
- **Decision (DataCamp/flexi/Upskilling).** Added to the same `EXTRA_ALLOWED_WORDS` set as ADR-051's abled/onlinemanipal/Coursera (proper nouns and non-dictionary jargon, unrelated to the dialect switch): "DataCamp" (third-party platform name), "flexi" (informal marketing coinage, "flexi-payment"), and "upskill"/"upskilling"/"upskilled"/"reskill"/"reskilling"/"reskilled" — genuinely modern workforce-education vocabulary too recent for this (or any general-purpose) Hunspell dictionary, and exactly the kind of term a career-focused EdTech platform uses constantly. This directly answers the user's "i dono what the issues with Upskilling" — it's a dictionary-coverage gap, same class as every other entry in this list, not a real spelling problem.
- **Verification.** Live-confirmed against the same `ln-pgcp-ba-mahe`/`ln-mba-mahe` runs used throughout ADR-047/051: DataCamp/flexi/upskilling/IoA/abled/onlinemanipal/Coursera all gone from both sides' flagged-word lists; American spellings (recognized, Organizations, Specialization, Behavior, Visualization, unauthorized, authorized) now correctly appear as flagged misspellings, confirming the dialect switch took effect both ways (not just "stop flagging British spelling" but genuinely "start checking against British English"). 6 tests rewritten/added in `spellCheck.test.ts` (British spellings via the dictionary itself, not an allowlist; an American spelling now flagged; DataCamp/flexi added to the existing word-allowlist test; upskilling/reskilling; the generalized mixed-case-acronym rule covering both IoA and MSc/PhD in one test). Full suite: 238 (website-quality).
- **Reverted the same day — see ADR-053.** The dictionary decision here (British-primary) did not survive contact with the site's actual content.

---

## ADR-053: Spell check reverts to US English — the British-primary switch (ADR-052) made things worse (2026-09-03)

- **Context.** Directly reversing ADR-052, same day: after seeing that switch's live results — American spellings across the page ("recognized", "Organizations", "Specialization", "Behavior", "Visualization", "unauthorized", "authorized"...) all newly flagged as misspellings — the user said "SO it should follow the English US spellcheck and not british English." The institution's actual page content turned out to be predominantly American-spelled (despite genuinely containing some British words too, e.g. "Amongst"), so making British the primary/only dictionary produced far more new false positives than it fixed.
- **Decision.** Reverted to `dictionary-en` (US) as the checked dictionary, and restored `BRITISH_SPELLING_ALLOWLIST` (the same ~37-word hand-curated list from ADR-047/051, plus "visualisation"/"visualisations" added while re-deriving it) to keep recognizing the genuine British words that do still turn up on this site without making British the default assumption. The two genuinely dialect-independent improvements made during the British-English detour were kept as-is: the generalized mixed-case-acronym rule (`isAcronymOrCode`, covers IoA/MSc/PhD-shaped tokens) and `EXTRA_ALLOWED_WORDS` (abled/onlinemanipal/Coursera/DataCamp/flexi/upskilling.../reskilling...) — neither has anything to do with US vs. British spelling, so neither needed touching.
- **Verification.** Live-confirmed against the same `ln-pgcp-ba-mahe` run: Master spell-check count dropped to 0 (previously flagged American words all gone), all of ADR-052's dialect-independent fixes (DataCamp, flexi, IoA, abled, onlinemanipal, Coursera, upskilling) still correctly exempted. 2 tests in `spellCheck.test.ts` updated back (the "no misspellings" baseline text reverted to American spelling; the American-spelling test now asserts NOT flagged instead of flagged) + 1 test fixture typo (a British "instalments" in an unrelated fixture, not itself under test, corrected to "installments" so that test's baseline is genuinely clean either way). Full suite: 238 (website-quality).

---

## ADR-054: Eligibility ground truth from the user's Excel — same pattern as Fee Structure (2026-09-07)

- **Context.** User's request: "need to map eligibility criteria from this excel file. its so clean that for UG and PG. kindly check into it and update from here" (`Online_Manipal_Banner_Eligibility_Corrected.xlsx`). Unlike the fee spreadsheet, this one is a verified *snapshot of what each Master page's own banner already says* (its own "Source & Scope" sheet: extracted from each course's landing page banner, checked 4 September 2026) — not a different, more-authoritative number than the page, but a hand-verified, trustworthy copy of exactly what the page states, more reliable than this tool's own live extraction of that same text (which `buildEligibilityField`'s own doc comment already documents as fragile on a real page — tab/accordion text, a stray tab-button-label leak). Same "the user's own verified file wins over live extraction, once a program is covered" principle already established for Fee Structure (ADR-040/043).
- **Structure (re-derived by hand, no Python in the dev environment — same unzip+XML method as the fee spreadsheet).** 3 sheets: `Course-wise Banner Eligibility` (the ground truth: Course | Level (UG/PG) | MUJ / SMU / MAHE banner-eligibility text, one column per institution, blank where that institution doesn't offer the course or wasn't checked), `UG-PG Same Eligibility` (a supporting analysis noting eligibility wording is NOT identical across every course at the same level for a given institution — informational only, not itself ground truth), `Source & Scope` (documents the extraction method/date).
- **Decision.** New `packages/core/src/data/eligibility-ground-truth.json`, keyed by resolved Master URL exactly like `fee-ground-truth.json`, `{program, level, eligibility}` per entry — 26 of the fee-ground-truth file's 27 URLs are covered (`eligibilityGroundTruthFor` in `data/index.ts`). `buildEligibilityField` gained an optional `masterUrl` parameter (default `""`, zero behavior change for every existing caller); when covered, the Master side's `text`/`evidence` are replaced by the spreadsheet's exact wording before the existing sub-fact decomposition/comparison logic runs unchanged — same override-the-resolved-value approach as `groundTruthMasterOverrides` for fees, not a parallel comparison path.
- **Two known gaps, not filled in.** (1) `online-ba-degree-smu` (SMU BA) has no eligibility row in the spreadsheet at all — falls back to normal Master-page-text comparison, same as any uncovered program. (2) Two real MAHE programs the spreadsheet does cover (MSc Financial Economics, MSc Biostatistics) have no entry here — this tool has no verified Master URL for either yet, and guessing one risked silently mapping the eligibility text to the wrong (or a nonexistent) page. Flagged to the user rather than guessed.
- **Verification.** Live-confirmed against the real MUJ MBA page (`online-mba-manipal-university-jaipur`, self-compared as its own target): `masterValue` reads "graduation · 50%" (derived from the spreadsheet's "Min 50% in graduation"), with the evidence excerpt explicitly reading `Verified against the user's eligibility spreadsheet (ground truth): "Min 50% in graduation"` — not scraped from the live page. 5 new tests in `priorityComparison.test.ts`: spreadsheet text wins over a deliberately-wrong Master-page claim; a genuine Target mismatch against the spreadsheet still reports correctly; the evidence excerpt is honestly labeled; an uncovered Master URL falls back unaffected; omitting `masterUrl` entirely (every pre-existing caller) behaves exactly as before. Full suite: 407 (core) + 238 (website-quality).
...
## ADR-056: Specializations — two more unrelated sections were winning SPECIALIZATION by content shape (2026-09-07)

- **Context.** User's request: "in Specializations / Combinations you need to match only the heading and enter the status... just check only the headings and match that all are same and write the status accordingly." Investigating this against a real MBA MAHE run (`ln-mba-mahe`) found exactly the class of bug this describes: the Target side's Specializations field reported `MATCH` with a `targetValue` of "TOP UNIVERSITIES, DOMAIN, SKILLS YOU'LL LEARN, Strategic Management, Cost Accounting, and 22 more" — visibly not a specialization list at all.
- **Root cause.** The rule-based semantic classifier's content-shape signal for SPECIALIZATION (a short list of title-cased, non-numeric items — the same heuristic that correctly recognizes a genuine keyword-less specialization list, e.g. "What are the MBA course subjects?") was ALSO winning on two entirely unrelated, generic EdTech-marketing sections: "Additional tools & certifications" (an external tool/platform-partnership widget, its own UI-chrome labels mixed with genuine tool/skill names) and "Who should pursue this certification?" (a target-audience/persona FAQ answer — job titles a program is marketed at, e.g. "Business leaders", "Entrepreneurs"). Both pass the exact same "short, title-cased, digit-free" shape check a real specialization list does — this is the same recurring failure class this classifier has been patched for repeatedly (Foundation Courses, Career Options, Skill Enhancement, Faculty, Alumni, Lead-Capture — see each pattern's own doc comment in `ruleBasedClassifier.ts`), just two more specific sections with real evidence.
- **Decision.** Two more narrow, evidence-based heading-exclusion patterns added to `NON_SPECIALIZATION_CONTENT_HEADING_PATTERN` (`packages/core/src/semantic/ruleBasedClassifier.ts`), same discipline as every existing entry: `TOOLS_CERTIFICATIONS_HEADING_PATTERN` and `TARGET_AUDIENCE_HEADING_PATTERN` (covers "who should pursue/take/join...", "who is this course for", "ideal for", "target audience"). Scoped like the others already in that combined pattern — removes SPECIALIZATION's content-shape signal only, doesn't gate the whole section (no evidence either falsely triggers another category via keyword collision), and a genuine specialization list under a different real heading is unaffected.
- **Bonus find: the same "Who should pursue this certification?" FAQ heading also exists on the real MAHE MBA Master page**, not just Target — this was, in fact, the ACTUAL cause of a previously-known, previously-accepted limitation this project's own `realHealthcareSpecializationRegression.test.ts` had explicitly documented and left unfixed (its old comment attributed the pollution to a "Get a Prestigious MBA Degree" heading, which turned out to be an imprecise diagnosis from whoever wrote it — the real 8-item pollution source was this FAQ answer). That test's expectation is updated from `PARTIAL` to `MATCH`, with its doc comment corrected to name the real cause — not a masked regression, a genuine fix confirmed by direct data inspection of the actual fixture.
- **Verification.** Live-confirmed against the real `ln-mba-mahe` run: Master and Target now both report the identical, clean 8-item specialization list ("Healthcare, Pharmaceutical Management, Finance, Business Analytics, Marketing, and 3 more") with status `MATCH` — no more UI-chrome or audience-persona pollution on either side. 4 new tests in `ruleBasedClassifier.test.ts` (tools & certifications excluded; target-audience excluded; 3 phrasing variants of the target-audience pattern; the MAHE MBA regression's own real keyword-less case still wins as before). Full suite: 415 (core) + 238 (website-quality).
...
## ADR-058: Accreditation "identical text still shows NEEDS_REVIEW" bug, and a Course Curriculum false-positive noise item (2026-09-09)

- **Context.** Two user reports in one message. (1) "kindly check all accrediation part because it shows it not matching but i cant find the where its not matching, i can see all are matching." (2) "For course curriculum we need to match only the headings and update the status" — the same request already made, and fixed, for Specializations (ADR-056).
- **Accreditation root cause.** `buildFactListPriorityField` (shared by Accreditation and Rankings & Accreditations, `packages/core/src/comparison/priorityComparison.ts`) downgrades a field to `NEEDS_REVIEW` whenever EITHER side has any claim that couldn't be broken into individual structured facts (a long marketing sentence with no recognizable "NAAC"/"AICTE"/etc. phrase). The old check only asked "did unstructured text exist on EITHER side", never "does it actually say the same thing on both sides" — so a real page pair (MAHE MBA Healthcare/Pharmaceutical Management) whose Accreditation section had every structured fact matching AND an extra marketing sentence that was word-for-word IDENTICAL on both Master and Target still got forced to NEEDS_REVIEW, with nothing for the user to actually find wrong. Live-confirmed: `masterValue`/`targetValue` in the report were the exact same string, yet status wasn't MATCH.
- **Accreditation fix.** `splitFactPhrases` now returns the actual unstructured text (normalized: trimmed, whitespace-collapsed, lowercased) instead of just a boolean flag. `buildFactListPriorityField`'s downgrade now fires only when the two sides' unstructured text sets actually differ (`sameUnstructuredText`, order-independent) — identical unstructured text on both pages is treated as a genuine match (nothing to review); differing unstructured text still correctly triggers NEEDS_REVIEW, since that's the one case a human genuinely can't verify from the structured facts alone.
- **Course Curriculum root cause.** Course Curriculum already worked exactly like Specializations post-ADR-056 (matches individual curriculum item NAMES via the semantic classifier's CURRICULUM/PROGRAM_STRUCTURE categories, not full paragraph content) — so there was no "compare only headings" redesign needed. But investigating live turned up a real false-positive noise item: on the MBA Pharmaceutical Management page, the Target's Course Curriculum value led with the bogus item "Benefit from our". Root cause: a "Develop core competencies" USP-strip card's own intro sentence ("Benefit from our expertly designed course curriculum that covers key subjects such as...") is chopped by the page's own markup into many short standalone DOM fragments ("Benefit from our", "curriculum", "management"...), each individually short enough to pass `classifySection`'s "only SHORT body text counts as a keyword signal" guard — so the single-word fragment "curriculum" alone won CURRICULUM classification for the whole USP card, alongside the page's real, separately-headed curriculum list (found correctly, HIGH confidence, under "Online MBA Course curriculum").
- **Course Curriculum fix.** New `PROGRAM_USP_BANNER_HEADING_PATTERN` (`packages/core/src/semantic/ruleBasedClassifier.ts`) gates the page's "why choose this program" USP-strip headings ("Up your career trajectory", "Broaden your skillset", "Develop core competencies", "Establish high-paying career") from EVERY classification signal, not just content-shape — this collision is a body-KEYWORD match, so it needed the full-section gate (same mechanism as `ALUMNI_STORIES_HEADING_PATTERN`/`LEAD_CAPTURE_FORM_HEADING_PATTERN`), not the narrower per-category exclusion `NON_SPECIALIZATION_CONTENT_HEADING_PATTERN` uses.
- **Verification.** Live-confirmed against the real MBA Pharmaceutical Management run: Accreditation is now `MATCH` (was `NEEDS_REVIEW`) with clean identical text on both sides; Course Curriculum's Target value now leads with "Managing People & Organizations..." — the same genuine list as Master, no more "Benefit from our" noise. 2 new priorityComparison tests (identical vs. differing unstructured text) plus 2 corrected pre-existing tests; 2 new ruleBasedClassifier tests (USP-strip exclusion; the genuine curriculum heading still wins). Full suite: 419 (core) + 241 (website-quality) — both green.
- **Also this session:** "divyang"/"divyaang" (the Hindi term for "differently abled", used in this site's own fee-discount category text) and "pharma" (short form of "pharmaceutical") added to spell check's `EXTRA_ALLOWED_WORDS` per the user's direct request, alongside these fixes.
...
## ADR-060: Accreditation/Rankings — the recurring benefits-strip and USP-banner contamination, finally traced and fixed (2026-09-17)

- **Context.** User's request: "Still i face lot of miss match in accration part. kindly check the back end i need you to check the Logo of each Accreditations and the ranking which is mentioned at the top of the logos" — with a concrete example pair: `distance-mcom-smu` (Target) vs. `online-mcom-degree-smu` (Master). This is the same "marketing content bleeding into Accreditation/Rankings" issue flagged (but not yet fixed) at the end of an earlier session, now given a specific reproducible example to chase down.
- **Three distinct contamination sources, all confirmed live on this one page pair.**
  1. A generic "why choose us" benefits strip (four icon-labeled items: UGC-entitlement, industry-webinar-access, a scholarship percentage, an alumni-network/legacy claim, each with its own one-sentence expansion, and several label+sentence concatenation variants of each) sits embedded **directly inside the page's real "Rankings & Accreditations" section by DOM position** — no separate heading of its own, so none of the existing heading-level exclusions could catch it. Present on the Target page only in this example.
  2. A "Get a prestigious [program] degree" USP card ("Globally recognized", "At par with on-campus degrees") — its own separate heading, present on both pages this time (worded slightly differently per program name), still inflating the field's unstructured-text diff.
  3. A job-market-recognition FAQ answer ("Will the online MCom be recognized while applying for jobs?"), present only on Master, chopped by the page's own markup into unreadable fragments ("Yes, Sikkim Manipal University's online", "MCo", "m", "MCo: m") — winning ACCREDITATION via body keyword ("UGC-entitled", "recognized").
- **Fix for #2 and #3 — full-section heading gates.** Two new patterns in `packages/core/src/semantic/ruleBasedClassifier.ts`, wired into `classifySection`'s existing full-gate check (same mechanism as `ALUMNI_STORIES_HEADING_PATTERN`/`PROGRAM_USP_BANNER_HEADING_PATTERN` — the collision is a body-keyword match, not content-shape, so it needs every scoring signal skipped, not just one): `PRESTIGIOUS_DEGREE_BANNER_HEADING_PATTERN` (matches "Get a prestigious degree" and "Get a Prestigious MCom/MBA/... Degree" — any one program-name word in between) and `JOB_RECOGNITION_FAQ_HEADING_PATTERN`. Note: the "Get a Prestigious MBA Degree" heading was ALSO named (misdiagnosed) as the root cause of an unrelated Specializations bug in ADR-056 — it wasn't (that bug's real cause was `TARGET_AUDIENCE_HEADING_PATTERN`); this is a separate, independently-confirmed exclusion for the ACCREDITATION contamination this heading's content genuinely does cause.
- **Fix for #1 — item-level filtering, not heading-level.** Since this content shares the page's real "Rankings & Accreditations" heading, no heading-gate can separate it. Extended the existing generic, cross-field `isPageChromeNoise` denylist (`packages/core/src/normalization/pageChromeNoise.ts` — already documented as applying "for any of the six primary fields... regardless of which heading it happens to sit under") with patterns covering every concatenation style of the four benefits-strip items (bare label, label+sentence run together, "label: sentence", sentence alone) — phrased around the generic CONCEPT (a webinars/scholarship-percentage/alumni-legacy benefit claim) rather than hardcoding the exact institution name, in the same spirit as the file's existing generic KYC-field-label entries. **Wired `isPageChromeNoise` into `splitFactPhrases`** (`priorityComparison.ts`) for the first time — it was previously only called from the Specializations content-shape scorer, never from Accreditation/Rankings' own extraction path, which is the actual gap that let this content through despite the filter already existing for a different field.
- **Verification.** Live-confirmed against the user's exact pair: Accreditation and Rankings & Accreditations both now report `MATCH` (previously `PARTIAL`/`NEEDS_REVIEW`) for `distance-mcom-smu` vs. `online-mcom-degree-smu`, with the genuine ranking/accreditation facts (NAAC A+ grade, UGC-entitlement, all 8 ranking lines) intact and identical on both sides. 7 new `isPageChromeNoise` tests, 2 new classifier tests (both new heading gates, including the not-caught-before genuine-content check), 1 new `priorityComparison` test (benefits-strip claim present only on Target no longer affects status when the real facts already match). Full suite: 429 (core) + 241 (website-quality) — both green.
- **Same-day follow-up — generalizing #3 past one named FAQ wording.** User: "we need to check only this part" (the Rankings & Accreditations logo-carousel widget itself), with a screenshot of exactly that widget — while re-verifying more pages, a SECOND, differently-worded compliance FAQ ("Is this online MCA program compliant with AICTE norms?", on a different SMU program page) hit the exact same bug `JOB_RECOGNITION_FAQ_HEADING_PATTERN` was named for. Naming each new FAQ wording one at a time doesn't scale, so this was generalized: a new `isFaqQuestionHeading()` check (any heading ending in `?`) now skips ACCREDITATION/RANKINGS scoring entirely for that section, checked per-category inside `classifySection`'s main scoring loop — narrower than the full-section gates above (a question-shaped FAQ heading stays a legitimate content source for OTHER fields, e.g. the real MAHE MBA regression's own "What are the MBA course subjects?" heading for SPECIALIZATION) but broad enough to catch every future differently-worded compliance/recognition FAQ without a new named pattern each time. The real widget's own heading is always the plain label "Rankings & Accreditations", confirmed never phrased as a question across every program page checked so far. 4 new tests (generalizes past the one named FAQ; scoped to just these two categories, not every category; the real widget heading still wins as before). Live-confirmed: the `online-mca-smu`/`online-mca-degree-smu` pair (previously the remaining differentiator after the first round of this ADR's fixes) now also reports `MATCH` on both fields. Full suite: 432 (core) — green.
- **Same-day follow-up #2 — an explicit hard constraint, not another denylist entry.** User: "you need to check only the carosal part on both the URL master and target. There you should check the images and rank mentioned on the top of some images only that... dont compare any others." Every fix above was still reactive — find one new marketing-copy pattern, add one new exclusion, repeat. The user's constraint asks for something stronger: a hard guarantee that ONLY the carousel's own items (a rank/accreditation-grade fact, or a short <=12-word caption) can ever affect this field's status, full stop. Removed the "differing unstructured text -> NEEDS_REVIEW" mechanism entirely (`sameUnstructuredText`, the `unstructuredTexts` tracking in `splitFactPhrases`, the downgrade in `buildFactListPriorityField`) — that mechanism was, in effect, STILL comparing non-carousel content, just refusing to call the result MATCH or UNMATCH. Now: a claim that doesn't match a rank/accreditation pattern and isn't short enough to plausibly be a caption is simply DROPPED — never compared, never flagged for review, regardless of whether it agrees or differs between the two pages. `splitFactPhrases` no longer returns an `unstructuredTexts` field at all; `buildFactListPriorityField` is now a two-line pass-through to `buildListPriorityField`. The benefits-strip filtering (`isPageChromeNoise`) and the heading gates from the fixes above remain as a belt-and-suspenders layer, not the only thing standing between noise and the report. 1 pre-existing test's expectation flipped (a differing long marketing sentence now correctly produces MATCH, not NEEDS_REVIEW, since it's no longer compared at all) with its doc comment corrected to explain why. Re-ran the full 72-URL batch: **NEEDS_REVIEW is now 0 for both fields** (down from 4, down from 12/19 at the start of this ADR); MATCH held steady at 28/30. Live-confirmed against a user-verified pair (`distance-ba-political-science` vs. `online-ba-degree-smu`, manually checked "matching... in all corousal images"): both fields report `MATCH`.
- **Same-day follow-up #3 — the MAHE subdomain's own differently-worded chrome.** User: "for Mahe there is an gear icon on target pages and the master pages i feel both are fine... always understand the context and not the exact texts." Investigating a real `mahe.onlinemanipal.com` (a genuinely different subdomain/template from `www.onlinemanipal.com`) pair found two more items forcing a false PARTIAL: a bare "MAHE" self-branding card (the institution's own name, no external rank/accreditation-body attached — never itself a comparable fact, since both pages in any comparison are already definitionally about the same institution) present on Master but absent from this subdomain's layout; and this subdomain's own differently-worded "why choose us" strip ("benefits", "At-par with on-campus degrees", "100% online experience", "Attractive scholarships", "Placement assistance"). Added to `EXCLUDED_FACT_PATTERNS` (`priorityComparison.ts`, scoped to Accreditation/Rankings only — NOT the cross-field `isPageChromeNoise` denylist — because "Placement assistance" would otherwise wrongly swallow the genuine Others-field "Placement / Career Support" sub-field). 2 new tests. Live-confirmed: the `mba-healthcare-manipal-academy-of-higher-education` (mahe.onlinemanipal.com) pair now reports `MATCH` on both fields.
- **On "rearranged rankings should still match."** The user also asked to treat reordered carousel items as a match, not a mismatch. Checked `compareTextItemList` (the underlying matcher, shared with Specializations) — it already matches items by normalized VALUE via a map lookup, not by position, so item order has never affected the result; nothing needed fixing there. Investigated one concrete "rearranged" example (`online-mba-muj` vs. its resolved master) to confirm nothing else was silently wrong: it turned out to be a genuine content-staleness issue, not a false positive — the target landing page shows OLDER ranking years (2023–2025: "Top 15 Universities (Management) 2023", "Top Universities in 2024") while the master page carries NEWER ones (2024–2026: "Top 10 Universities (Management) 2024", "Top 300 Universities (2026)") — correctly flagged as different, not a bug. No fix applied here; a specific URL where a truly identical-but-reordered/reworded ranking set is still being flagged would be needed to investigate further.

---

## ADR-061: A code-level Target→Master URL override, checked automatically in every run (2026-09-24)

- **Context.** The MAHE Healthcare/Pharmaceutical Management target→master mapping had been living only in this session's own memory notes (manually re-applied whenever a request happened to mention it explicitly). User: "everytime when we run with the list of URL these 2 URL should match with the master URLs i mentioned. Always remember that" — an explicit request for a durable, code-level guarantee, not something dependent on a future session recalling and re-typing the right Master URL by hand.
- **Decision.** New `packages/core/src/data/master-url-overrides.json` (same ground-truth-file pattern as Fee/Eligibility/Duration — see `data/index.ts`'s `masterUrlOverrideFor()`), keyed by exact Target URL, exported from `@crosscheck/core`'s public barrel (`index.ts`) for the first time — every other ground-truth loader in that module was previously internal to `packages/core` only; this one needs to be consulted from `modules/website-quality`. Checked at the very top of target resolution (`resolveOneTarget` in `discoverAndCompareMany.ts` — the function the actual API/batch path uses, NOT the separate, currently-unused `resolveAuthoritativePage.ts`, whose own parallel Sprint-5 implementation was updated too for consistency but doesn't run in production) — right after the target's own identity is resolved (still needed for report display) but before the registry/dynamic-discovery path or the identity-conflict check, since an override is meant to bypass exactly the kind of resolution failure a conflict represents. New `AuthoritativePageResolutionMethod`/`TargetResolutionMethod` value `"manual_override"`.
- **Contains 3 confirmed pairs** (carried over from memory into code): the `mahe.onlinemanipal.com` subdomain healthcare page (ADR from 2026-09-10), and two shorter-slug `www.onlinemanipal.com` pages — `online-mba-healthcare-mahe` → `online-mba-healthcare-management-mahe`, `online-mba-pharma-mahe` → `online-mba-pharmaceutical-management-mahe`.
- **Verification.** Live-confirmed: submitting a batch run with ONLY the generic root Master (`https://www.onlinemanipal.com/`) and both new Target URLs, neither passed a Master explicitly, both resolved via `method: "manual_override"` to exactly the right Master URL — the guarantee now holds for any run these Targets appear in, including a large batch list, not just a manually-constructed one-off comparison. 1 new end-to-end test (`discoverAndCompareMany.test.ts`) proving the override wins even against a deliberately different, unrelated top-level Master. Full suite: 434 (core) + 242 (website-quality) — both green.

---

## ADR-062: Fee Structure — a Target's USD price no longer wins over its own INR price (2026-09-24)

- **Context.** User: "for 2 URL its matching USD price. kindly check it needs to search always INR price and match thats why it is showing NEEDS REVIEW." `onlinemanipal.com`'s MAHE MBA "working professionals" page has an Indian-students/International-students tabbed fee widget — both tabs' amounts become fee candidates for the same page.
- **Two compounding root causes, both live-confirmed on the same page.**
  1. `resolveFeeComponentSide` (`priorityComparison.ts`) returned the FIRST candidate that normalized to a clean number, regardless of currency — a USD candidate could win over an INR one purely because of extraction order, even though Master's own figure is always INR (every Indian university's own program fee).
  2. The page's genuine INR "Full Fee" text states BOTH the original and discounted amount in one combined string ("Full Fee Payment: INR 2,92,000 INR 2,77,400*", no `<del>`/`<s>` structural split this tool's discount-detection otherwise relies on) — `normalizeClaim` correctly refuses to guess between two numbers for one currency (`AMBIGUOUS`), which threw the ENTIRE claim out, so the genuine INR figure never even reached the "prefer INR" check — it fell through all the way to the separate, cleanly-single-number USD candidate.
- **Fix.** (1) INR is now always preferred: the search keeps scanning past a non-INR confirmed match instead of returning it immediately, only falling back to it at the end if no INR match exists anywhere among the candidates. (2) New `splitRepeatedCurrencyAmounts()` — when a claim fails to normalize, retries against just its first currency-prefixed-number piece (found by locating every currency-symbol occurrence in the text and slicing between them), so the combined "original INR amount, discounted INR amount" string's first (larger, undiscounted) figure gets a real chance to normalize instead of the whole claim being discarded.
- **Verification.** Live-confirmed against the real page: "Full Fee" now reports `MATCH` (Target: "INR 2,92,000", was "USD 3,800" via `NEEDS_REVIEW`/"different currency"). A genuinely USD-only page (no INR figure present at all) still correctly reports `NEEDS_REVIEW` with a "different currency" note — the fix doesn't paper over a real currency-only page, it only stops a real INR figure from being passed over. 3 new tests (INR preferred over USD when both present; USD-only correctly stays NEEDS_REVIEW; the combined "INR X INR Y" real-page case). Full suite: 437 (core) — green.

---

## ADR-063: Credits (new 8th primary row) + Course Curriculum's elective-menu noise, three iterations to a real fix (2026-09-24)

- **Context.** User: "And i need one more column called credits and needs to check the credits for each course. And course cariculam we need to check the headings for each semester and say the status. only headsing is enough." Live investigation found `onlinemanipal.com` pages never expose a per-course credit breakdown -- only one bare "NN Credits" badge naming the program's overall total -- and that a literal "heading per semester" comparison would be too thin to be useful (many pages don't consistently expose "Semester N" headings at all, see below). Both points were put to the user via `AskUserQuestion`: Credits scope -> **"Use the program total (Recommended)"**; Course Curriculum scope -> **"Keep comparing subjects too, just cleaner"** (filter obvious noise, don't reduce to headings-only).
- **Credits -- new field, straightforward.** New `extractCreditsClaim()` (`modules/website-quality/src/understanding/priorityExtraction.ts`) -- a whole-string-anchored `/^\d+\s*credits?$/i` scan over text blocks, since a bare "92 Credits" badge has no adjacent "Credits:" label and isn't under a heading literally named "Credits" (the two extraction mechanisms every other field already uses). Compared as an 8th scalar `buildScalarPriorityField` row (`PriorityReportFieldName` gains `"Credits"`), same mechanism as Course Duration. Dashboard's `NewRunPage` field list, `TargetTable` overview column, and `exportRunExcel`'s field list all updated to match. Live-confirmed: MATCH when both pages state the same total ("92 Credits" = "92 Credits"), UNMATCH naming both values when they genuinely differ, NEEDS_REVIEW (never a fabricated MATCH) when neither page states one.
- **Course Curriculum -- three iterations, each one a real live bug the previous one missed.** All three build on the fixed field set: an 8-row report where Course Curriculum keeps comparing actual subjects (per the user's second answer), just with elective-choice noise filtered out.
  1. **Label-based state machine.** A literal `"Specialization:"` label (or `"Semester N"` / `"Core Subjects"` / `"N modules as per chosen elective"` heading) toggles an `inElectiveMenu` state in a new `filterCurriculumFacts()` (`priorityComparison.ts`), dropping everything between a `"Specialization:"` label and the next semester boundary. Worked for the one page it was built against (Healthcare Management vs. `online-mba-degree-working-professionals-mahe`).
  2. **Cross-referencing the page's own Specializations list.** Re-verified live against a second pair (`online-mba-mahe`) and found its Master page's own extraction (210 facts) never produces a `"Specialization:"` label fact at all -- a different DOM structure than the page iteration 1 was built against -- so the state machine never triggered, letting every specialization-track name (Finance, Marketing, HRM, Business Analytics, Data Science...) leak through as if they were core subjects (23+ items falsely "missing on Target"). Fixed by cross-referencing each CURRICULUM fact against the page's own already-computed SPECIALIZATION facts (a field this report computes anyway) via `conceptsEquivalent` specifically -- its bounded synonym table (e.g. `"hrm"` <-> `"human resource management"`, exactly the case that first exposed this) -- deliberately NOT the fuzzier `tokensOverlapEnough` used elsewhere in this file, which would wrongly exclude a genuine core subject like "Marketing Management" for sharing the word "marketing" with the specialization "Marketing".
  3. **A lone name match must never open the menu by itself; only a back-to-back run does.** Iteration 2 still left "HRM" and ~22 other items leaking on the same page. Dumping the Master page's raw facts directly showed why: the 9 specialization-track names (Finance, Marketing, HRM...) appear as one back-to-back BLOCK immediately before their own elective courses, which themselves carry NO label or boundary of their own at all -- right up until the next "Core Subjects" heading. A single category-name match closing nothing meant its unlabeled electives kept leaking. New `findSpecializationHeaderRuns()` requires 2+ CONSECUTIVE specialization-name matches before treating them as a menu header (matching the real observed page shape); only then does it open the elective-menu state for what follows, closed again by the next `"Core Subjects"`-style heading. This run-length check also caught a second, opposite-direction live bug on `online-bba-mahe`: that page's own (separately noisy) Specializations extraction happens to include "Marketing Management" -- which is ALSO the literal title of a genuine Semester-1 core subject on that same page -- and with no run-length check, that single coincidental name collision opened the elective menu at the very first semester and (since the page has no "Core Subjects" heading anywhere to close it again) silently dropped every subject for the rest of the page.
- **Verification.** All three MAHE MBA/BBA pairs that motivated this ADR now report Course Curriculum `MATCH` live, with no falsely-dropped items: `online-mba-mahe` (was PARTIAL, "23 more missing"), `online-bba-mahe` (was PARTIAL, "4 more missing" after iteration 2's partial fix -- iteration 3 was needed to fully close it), `online-mba-healthcare-mahe`. 5 new `priorityComparison` tests covering all three iterations' specific real-page shapes (label-based menu, no-label cross-reference, synonym-only abbreviation match, back-to-back run opening/closing correctly, and the single-coincidental-match false positive). Full suite: 447 (core), 245 (website-quality) -- both green.
- **Same-day follow-up -- Credits missed a real value that WAS on the page.** User, seeing an UNMATCH in a 79-URL batch run: "credits it is there in both the URL... i can be mentioned any where in the website. SO check." Live-confirmed: `online-mba-mahe` genuinely states "92 credits" on screen, but folded into one pipe-separated quick-facts line ("24 months | 4 semesters | 15-20 hours/week | 92 credits") rather than its own bare badge -- the ONLY shape the original `CREDITS_BADGE_PATTERN` (whole-block anchor) could match, so the field wrongly reported it as absent on that page. Fixed by adding a second, fallback pattern (`CREDITS_EMBEDDED_PATTERN`, `\b(\d+)\s*credits?\b`) that finds "<number> credit(s)" anywhere within a block once the bare-badge check finds nothing, still requiring the digit immediately before the word so it doesn't fire on unrelated prose like "Academic Bank of Credits" or "meeting credit norms" (neither has a digit right next to "credit"). Extracts just the matched "NN Credits" portion (not the whole quick-facts line) so it compares like-for-like against a page that states it as its own bare badge. 1 new extraction test. Live-confirmed: `online-mba-mahe` and `online-mba-healthcare-mahe` now both report Credits `MATCH` ("92 Credits" = "92 Credits"); `online-bba-mahe`/`online-bba-honors-mahe` correctly stays `NEEDS_REVIEW` -- dumped every "credit"-mentioning text block on both pages directly and confirmed neither one actually states a numeric credit total anywhere, so that result is correct, not a miss. Full suite: 12 (priorityExtraction), 246 (website-quality) -- both green.

---

## ADR-064: Semester Fee false UNMATCH -- a conditional "Effective ... Fee" rate silently outranked the genuine matching figure (2026-09-25)

- **Context.** User: "Kindly check the Semester fee it was showing unmatch but when i check both the pages are same price its mentioned." The 79-URL batch run that surfaced this had already been lost (server restarted twice since, for the two prior same-day fixes -- `RunStore` is explicitly in-memory-only, see ADR-011), so this was re-diagnosed from a fresh batch of known MAHE pairs rather than the original run.
- **Root cause, live-confirmed on `online-mba-mahe` (Target) vs. `online-mba-degree-working-professionals-mahe` (Master).** Both pages genuinely state the identical undiscounted per-semester rate -- Master: "Semester Fee: INR 73,000"; Target: "EACH SEMESTER: INR 73,000*" -- so the report SHOULD show `MATCH`. But Target's page separately states a conditional, first-semester-only promotional rate: "Effective First Semester Fee INR 62,050" (from "15% Discount on 1st Sem Fee for all women & Corporate employees" -- 73,000 x 0.85 = 62,050). `classifyFeeText`'s `DISCOUNT_PATTERN` only recognized the literal words "discount"/"concession"/"% off" -- none of which appear in the "Effective First Semester Fee INR 62,050" sentence itself (the "15% Discount..." wording is a SEPARATE nearby fact) -- so this conditional rate was classified `discounted: false`, the same slot the genuine "EACH SEMESTER: INR 73,000*" figure was competing for. `resolveFeeComponentSide` (ADR-062) returns the FIRST confirmed match it finds scanning candidates in extraction order, and the conditional rate happened to appear earlier in that order, so it silently won -- producing a false `Target semester fee is INR 10,950 lower than Master` UNMATCH between two pages that actually agree.
- **Fix.** Added `\beffective\b` to `DISCOUNT_PATTERN` (`priorityComparison.ts`). "Effective <fee>" is a standard idiom in fee/pricing copy generically -- "the rate AFTER an adjustment is applied" -- not specific to this one institution's wording, so it belongs in the same discount-keyword class as "discount"/"concession"/"% off", same discipline as every other pattern in this file (generic keyword, no institution-specific vocabulary). This correctly reclassifies the conditional rate as `discounted: true`, moving it out of the "Semester Fee" (non-discounted) slot it was wrongly occupying and into "Semester Fee (After Discount)" instead, where it belongs semantically.
- **Verification.** Live-confirmed: `online-mba-mahe` now reports Semester Fee `MATCH` ("EACH SEMESTER: INR 73,000*" = "Semester Fee: INR 73,000"). Re-ran a 4-pair sanity batch (all MAHE MBA/BBA pairs touched by this session's other fixes) -- Full Fee, Semester Fee, and Monthly EMI all `MATCH` with no regressions; "Semester Fee (After Discount)"/"Full Fee (After Discount)" stayed `UNMATCH` exactly as before this fix on every pair (a pre-existing, ground-truth-spreadsheet-driven result unrelated to this change, not something this fix touches or the user raised). 1 new `priorityComparison` test reproducing the exact real-page scenario. Full suite: 448 (core) -- green.
- **Same-day follow-up -- the user asked for a fundamentally more robust mechanism, not another one-off keyword patch.** User, after being shown the diagnosis for `online-mba` vs. `online-mba-manipal-university-jaipur`: "you should check all the places and you should match it sometimes it should match multiple places as well so better you can say these many places are there with matched and not matched. or what you suggest." This was the THIRD time in one day the same failure shape recurred (ADR-062's USD-over-INR bug, this ADR's "Effective" bug, now a THIRD wording -- "Each semester fee: INR 38,250" -- carrying no discount-indicating keyword at all in that sentence, while the genuine undiscounted ₹45,000 sits elsewhere on the same page as a bare, unlabeled number the strict period-keyword classifier never associates with "semester"). Offered 3 design options via `AskUserQuestion`; user chose **"Match if found anywhere"**: a fee row is `MATCH` if Master's expected amount appears ANYWHERE among Target's fee-shaped mentions, not only under the ONE label/period/discount combination the strict classifier happens to prefer, with the road-not-taken mention named in the row's note rather than silently discarded.
  - **Mechanism.** New `resolveFeeComponentSideByAmount()` (`priorityComparison.ts`) -- given Master's already-resolved amount+currency, scans every Target candidate of the matching fee TYPE (ignoring period and discount classification entirely, since those two are exactly what every fee-matching bug this session has hit lived in) for one that numerically equals it, checking every piece of a combined "INR X INR Y" claim, not just the first. Used as a FALLBACK inside `resolveFeeComponentSubFacts`, engaged only when the strict pick doesn't already agree with Master -- a cleanly-labeled page's behavior is completely unchanged. `feeType` alone stays a guardrail against a genuine cross-concept collision (e.g. an Application Fee accidentally equal to a Semester Fee); the classifier is far more reliable at telling fee TYPES apart than at telling period/discount VARIANTS of the same type apart.
  - **Verification -- the full picture, not a single pair.** Live-ran the user's own exact 14 Target/Master pairs (every MUJ/SMU/MCA specialization landing page): **Semester Fee now `MATCH` on 5 of 14** (`online-mba`, `online-mba-muj`, `online-mba-smu`, `online-mca`, `online-mca-smu` -- pages that DO state the plain ₹45,000/₹30,000/etc. figure somewhere, previously all false `UNMATCH`), and **stays `UNMATCH` on the other 9** (every "-lp" specialization landing page, e.g. `online-mba-finance-lp`) -- confirmed by a direct raw-text dump that these 9 pages genuinely never state the undiscounted per-semester figure ANYWHERE at all (only the discounted rate and the derivable-but-never-literally-stated full-fee-divided-by-4 value), so `UNMATCH` there is correct, not a bug this fix should paper over by inferring an unstated number. Full Fee, Full Fee (After Discount), Semester Fee (After Discount), and Monthly EMI all stayed `MATCH` across all 14 pairs -- no regressions. 3 new `priorityComparison` tests (the broader match firing and naming the other mention; a cleanly-labeled page proving the fallback never engages when unneeded; a genuine absence still correctly reporting UNMATCH). Full suite: 451 (core) -- green.
- **Same-day follow-up #2 -- the "genuinely absent" verification above was WRONG; the real bug was a silent DOM extraction gap, not missing content.** User, pushing back with the 9 "-lp" URLs named individually: "check these URL. these are the URLS i saw its matching." Opened `online-mba-finance-lp` in a real browser (not this tool's own fetch) and confirmed the rendered page genuinely DOES show "FEES: INR 45,000 per semester" -- directly contradicting the previous follow-up's raw-text-dump conclusion. Fetching the SAME page's raw HTML directly found the real structure: `<span class="feeText">INR 45,000 <span>per semester</span></span>` -- the amount is the PARENT span's own direct text, sitting alongside a nested REAL (non-icon) child span holding "per semester". `extractMainTextAndBlocks`'s div/span extraction rule (`extract.ts`) treated ANY real element child as disqualifying the WHOLE element from capture, on the assumption its text would "either be a duplicate of a child's, or be captured when that child itself is visited" -- true for the nested child ("per semester" DID get its own separate capture), false for the PARENT's own text ("INR 45,000" was thrown away entirely, not duplicated anywhere). This is a genuine, silent extraction gap that had nothing to do with fee classification/matching logic at all -- the number was simply never seen, on some pages the ONLY place the page states its true undiscounted per-semester rate.
  - **Fix.** New `directOwnText()` helper -- unlike the existing `ownText()` (which still includes every descendant's text, just minus a struck-through discount price), this returns ONLY an element's direct text nodes, never a descendant's, so it can never duplicate a child's own separate capture. When a div/span has a real (non-icon) element child, its own direct text is now captured via `directOwnText` instead of being discarded outright; a purely decorative icon-only child (svg/img) still behaves exactly as before.
  - **Verification.** Re-ran the SAME 14 Target/Master pairs: **Semester Fee now `MATCH` on all 14 of 14** (up from 5 of 14) -- every "-lp" page the previous follow-up wrongly called a genuine content gap now correctly resolves. Full Fee, Full Fee (After Discount), Semester Fee (After Discount), and Monthly EMI stayed `MATCH` on all 14 pairs -- no regressions from a change to the shared, general-purpose DOM extraction path used by every field, not just fees. 4 new extraction tests (the parent's own text is captured; never duplicated with the child's; becomes a real feeCandidate claim; an icon-only child still behaves as before). Full suite: 451 (core), 250 (website-quality, up from 246) -- both green.
  - **Lesson (recorded, not just fixed): a "the page genuinely doesn't say this" conclusion drawn only from this tool's OWN extracted text is not verification -- it can be validating this tool's own bug.** The previous follow-up's raw-text dump LOOKED like independent confirmation but was actually just re-reading the same broken extraction pipeline it should have been checking. Opening the real page in a browser (or reading the raw HTML directly, bypassing this tool's parser entirely) is what actually caught it.

---

## ADR-065: Excel export's Overview sheet was missing on-screen dashboard columns (2026-09-25)

- **Context.** User: "when i download as excel file i am not getting all the fields" -- and pasted the dashboard's own column list. `TargetTable`'s overview replaced the single aggregated "Fee Structure" column with 4 fee-COMPONENT columns back when that redesign happened (2026-09-03), and separately gained a "Changed Fields" count column -- but `exportRunExcel.ts`'s `buildOverviewRows` (added later, 2026-09-11) was built against the OLDER field list and never updated to match, so the spreadsheet silently diverged from what the dashboard actually shows.
- **Fix.** Added a `FEE_COMPONENT_COLUMNS` constant to `exportRunExcel.ts` -- deliberately duplicating `TargetTable.tsx`'s own constant of the same name/shape (same 4 names, same labels, same order: Full Fee Payment, Semester Fee Payment, No-cost EMI / Monthly Payment, Discount (Full Fee)) rather than importing it, since one lives in a component file and the other in a plain data-shaping module -- and a "Changed Fields" column using the same `countChangedFields` helper `TargetTable` already uses, so the two can never silently disagree about what counts as a change. The pre-existing `PRIORITY_FIELDS`/`SECONDARY_FIELDS` columns (including the older aggregated "Fee Structure"/"Discount" rows the dashboard no longer shows on its overview, and "Others", which it also doesn't show) were left in place rather than removed -- more detail in the spreadsheet than the screen is a feature there, not a mismatch worth breaking.
- **Verification.** 2 new `buildOverviewRows` tests (Changed Fields counts mismatches only, matching `countChangedFields`'s own contract; all 4 fee-component columns present with a dash for a program with no discount component). Full suite: 7 (exportRunExcel), 108 (dashboard) -- both green; dashboard typechecks clean.
