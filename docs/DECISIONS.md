...
- **Root causes found (five, all in the same subsystem, diagnosed via live testing against the real site).**
...
- **Decision — five additive, registry-aware fixes, all optional/backward-compatible (absent `registry` param = zero behavior change for every pre-fix caller):**
...
- **Verification.** Iteratively live-tested against the real site throughout (`online-bba-mahe`, `online-mba-mahe`, `mahe.onlinemanipal.com/programs/mba-manipal-academy-of-higher-education`, `mahe.onlinemanipal.com/programs/mba-healthcare-manipal-academy-of-higher-education`, `manipaluniversity.co.in/online-mba-degrees`, `manipaluniversity.co.in/online-mca-degrees`, `manipaluniversity.co.in/online-bba-degrees`) after each fix, re-running the full suite after every change:
  - Before: MUJ's MBA page (score 90) beat MAHE's own genuine MBA page for a MAHE target; `online-mba-mahe`-style targets resolved to the wrong program (a BCom page) or failed outright; every `manipaluniversity.co.in`/`mahe.onlinemanipal.com` target either false-failed or matched the wrong institution.
  - After: all of the above resolve to their correct MAHE/MUJ candidate with a decisive score margin (e.g. 100 vs 40, 85 vs 25), except `manipaluniversity.co.in/online-bba-degrees` — whose own page genuinely never says which Manipal university it means — which now correctly reports `ambiguous_candidates` (honest ambiguity) instead of the previous false `authoritative_page_not_found`.
  - packages/core: 347/347 tests passing (8 new regression tests added, 2 stale test fixtures updated to reflect the more precise institution-conflict logic). modules/website-quality: 219/219 tests passing (2 new regression tests added). Zero net regressions across both suites at every step.
- **Not addressed / still open.** Institution matching now depends on `matchSpecificInstitution`'s exact whole-string equality; a target institution mention that's specific but phrased in a form not registered as an alias (e.g. a typo, or an unlisted abbreviation) would still resolve to "unresolved"/ambiguous rather than matched — by design (never guess), but worth revisiting if it recurs across the 89-target batch. The user's second, separate ask (an easier way to review ~90 target reports without opening each one individually) is addressed by the existing "All Reports" combined page (`/runs/:runId/report`); not yet re-confirmed with the user whether that fully satisfies the ask.
