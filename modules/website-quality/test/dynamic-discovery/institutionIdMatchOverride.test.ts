...
describe("evaluateInstitutionGateForPair -- canonical institutionId override", () => {
  it("a confidently-resolved matching institutionId on both sides overrides a raw text conflict between two specific institution names", async () => {
    // 2026-08-21 note: this pair must be two EXACT, different registered
    // institution names -- text this specific still produces a genuine
    // "conflict" verdict even under the smarter, registry-aware
    // `institutionOrBrandVerdict` (a vague/partial mention like "MAHE
    // Online" no longer counts as a conflict on its own; see
    // institution-relevance.test.ts's 2026-08-21 fix tests).
    const target = signals({ institution: { value: "Manipal Academy of Higher Education", confidence: "medium", matchedSignals: [] } });
    const candidate = signals({ institution: { value: "Sikkim Manipal University", confidence: "medium", matchedSignals: [] } });
  ...
  // 2026-08-20 fix -- the symmetric "confident mismatch" override. Live-
  // confirmed on onlinemanipal.com: header logo, footer text, and the
  // generic institution meta tag are theme-level constants IDENTICAL
  // across the entire site regardless of which specific university a page
  // is about -- so raw text/logo signals alone had nothing left to reject
  // an unrelated-institution candidate with, once BOTH sides independently
  // (and confidently) resolved to specific, DIFFERENT institutionIds via
  // URL/logo evidence.
  it("identical shared-portal-brand text on both sides, which the old text-only gate would have let pass, is force-rejected once both sides independently resolve to different institutionIds", async () => {
    const target = signals({ institution: { value: "Online Manipal", confidence: "medium", matchedSignals: [] } });
    const candidate = signals({ institution: { value: "Online Manipal", confidence: "medium", matchedSignals: [] } });

    // Proves the premise: identical raw text alone passes the gate (the old bug).
    const withoutInstitutionIdentity = await evaluateInstitutionGateForPair(target, candidate, DEFAULT_INSTITUTION_RELEVANCE_GATE_CONFIG, neverResolveLogoHash);
    expect(withoutInstitutionIdentity.passed).toBe(true);

    const withDifferentResolvedIds = await evaluateInstitutionGateForPair(
      target,
      candidate,
      DEFAULT_INSTITUTION_RELEVANCE_GATE_CONFIG,
      neverResolveLogoHash,
    ...
    expect(withDifferentResolvedIds.passed).toBe(false);
    expect(withDifferentResolvedIds.signals.institutionOrBrand).toBe("conflict");
  });

  it("the mismatch override does not fire when only one side's institutionId is resolved -- falls back to the pre-existing text/logo gate unchanged", async () => {
    const target = signals({ institution: { value: "Online Manipal", confidence: "medium", matchedSignals: [] } });
    const candidate = signals({ institution: { value: "Online Manipal", confidence: "medium", matchedSignals: [] } });
